...
public:
    ...
    /// 运算符重载，x和y全是int类型，在这里作运算符重载，其他函数没重载，有什么意义？
    /// 明白了，省事了，直接用运算符重载进行x和y的比较，直接返回一个bool，不用再单独比较了。
    // TODO: add this in tricks
    bool operator == (const Node2D& rhs) const;

    // GRID CHECKING
    /// Test if the node is inside the map
