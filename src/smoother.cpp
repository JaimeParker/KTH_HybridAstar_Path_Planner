...
void Smoother::tracePath(const Node3D* node, int i, std::vector<Node3D> path) {
    // loop until the first node whose pred is a nullptr
    if (node == nullptr) {
        this->path = path;
        return;
    }
...
Vector2D Smoother::curvatureTerm(Vector2D x_im2, Vector2D x_im1, Vector2D x_i, Vector2D x_ip1, Vector2D x_ip2) {
    ...
    if (delta_x_im1.length() > 0 && delta_x_i.length() > 0 && delta_x_ip1.length() > 0 && delta_x_ip2.length() > 0) {
        ...
        if (kappa_i <= kappaMax) {
            ...
            const Vector2D& d_cos_delta_phi_i = delta_x_ip1.ort(delta_x_i)
                    / (delta_x_ip1.length() * delta_x_i.length())
                    - delta_x_i.ort(delta_x_ip1)
                    / (delta_x_i.length() * delta_x_ip1.length());
            ...
            const Vector2D& d_cos_delta_phi_ip1 = -delta_x_ip2.ort(delta_x_ip1)
                    / (delta_x_ip2.length() * delta_x_ip1.length());
