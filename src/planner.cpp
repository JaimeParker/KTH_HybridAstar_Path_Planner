...
Planner::Planner() {
    ...
    // TOPICS TO SUBSCRIBE
    if (Constants::manual) {
        // subscriber that call function setMap
    ...
    // subscriber that call function setGoal
    subGoal = n.subscribe("/move_base_simple/goal", 1, &Planner::setGoal, this);
    // subscriber that call function setStart
...
void Planner::setMap(const nav_msgs::OccupancyGrid::Ptr map) {
    ...
    // won't be used if manual is set as true, constant variable
    if (!Constants::manual && listener.canTransform(
          "/map", ros::Time(0),"/base_link",
...
void Planner::plan() {
    ...
    if (validStart && validGoal) {
        // if things go well, proceed here
